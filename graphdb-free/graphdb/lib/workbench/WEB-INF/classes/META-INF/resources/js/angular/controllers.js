define(['angular/core/services'], function () {
    ...
    homeCtrl.$inject = ['$scope', '$http', '$location', 'ClassInstanceDetailsService', 'AutocompleteService'];
    function homeCtrl($scope, $http, $location, ClassInstanceDetailsService, AutocompleteService) {
        $http.get('graphdb-settings/license/hardcoded')
            ...
            .then(function () {
                $http.get('graphdb-settings/license').then(function (res) {
    ...
    function mainCtrl($scope, $menuItems, $jwtAuth, $http, $cookies, toastr, $location, $repositories, $rootScope, localStorageService, productInfo, $timeout, ModalService, $interval, $filter) {
        ...
        $scope.$on("$routeChangeSuccess", function (event, next, current) {
            if ($location.path() === '/' && $scope.getActiveRepository() !== '') {
                $scope.getRepoSize($scope.getActiveRepository());
                $scope.setYears();
            }
            $scope.clicked = false;
        ...
        //Change repository
        $scope.changeRepository = function ($event, id) {
            $repositories.setRepository(id);
            $scope.getRepoSize(id);
        };
        ...
        $scope.popoverTemplate = 'v/8.2.0/js/angular/templates/repositorySize.html';
        ...
        $scope.canWriteActiveRepo = function (noSystem) {
            ...
            return $jwtAuth.canWriteRepo($repositories.getActiveLocation(), $repositories.getActiveRepository())
                && ($repositories.getActiveRepository() !== 'SYSTEM' || !noSystem);
        };

        $scope.getActiveRepositoryType = function () {
            if ($scope.getActiveRepository() && $scope.getRepositories()) {
                for (var i = 0; i < $scope.getRepositories().length; i++) {
                    if ($scope.getRepositories()[i].id == $scope.getActiveRepository()) {
                        return $repositories.getRepositories()[i].type;
                    }
                }
            }
            return false;
        ...
        $scope.getRepositorySize = function (repository) {
            if ($scope.popoverRepo) {
                $http.get('rest/repositories/' + $scope.popoverRepo + '/size').then(function (res) {
                    $scope.size = res.data;
                });
            } else {
                $scope.size = {};
        ...
        $scope.getRepoSize = function (repoId) {
            if (angular.isUndefined(repoId)) {
                return;
            }
            $scope.repoSizeError = undefined;
            $http.get('rest/repositories/' + repoId + '/size').then(function (res) {
                $scope.repoSize = res.data;
            }).catch(function (e) {
                $scope.repoSizeError = e.data.message;
            });
        };
        ...
        $scope.goToSparqlEditor = function (query) {
            $location.path('/sparql').search({savedQueryName: query.name, execute: true});
        };
        ...
        $scope.getNumberOfActiveImports = function () {
            ...
            $http({
                method: 'GET',
                url: 'rest/data/import/active',
                params: {repository: $scope.getActiveRepository()},
                timeout: 10000
            }).success(function (data, status, headers, config) {
        ...
        $scope.checkLicenseStatus = function () {
            $http.get('graphdb-settings/license/hardcoded').success(function (res) {
                $scope.isLicenseHardcoded = (res === "true");
            }).error(function () {
                $scope.isLicenseHardcoded = true;
            }).then(function () {
